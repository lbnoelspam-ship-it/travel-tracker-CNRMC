...
if date_sequencing_valid and origin_geo and FEDERAL_RATES_DB and len(legs_data) == st.session_state["num_legs"]:
    ...
    flight_chain = [{"name": origin_geo["clean_name"], "lat": origin_geo["lat"], "lon": origin_geo["lon"], "is_foreign": origin_geo["is_foreign"]}]
    for leg in legs_data:
        flight_chain.append(leg)
    flight_chain.append({"name": origin_geo["clean_name"], "lat": origin_geo["lat"], "lon": origin_geo["lon"], "is_foreign": origin_geo["is_foreign"]})
    ...
    ledger_df = pd.DataFrame([
        {"Category": "Airfare", "Estimated Cost": round(total_airfare_cost, 2), "Details": "Tiered flight cost calculation based on flight distance and routing"},
